...
Use `@flexiboards/testing` (a dev dependency) rather than hand-rolling geometry stubs. happy-dom and jsdom have no layout, so a board mounted there cannot resolve a pointer to a cell. In a Vitest setup file call `installResizeObserver()` and `configure({ flush })`: for Svelte, `flush` runs the work then `flushSync()`; for React, pass `act`. In a test: mount the board, call `layoutGrid(100)` (it sizes the board, grid and every cell from their aria attributes and fires the resize observers), then drive it with `grabByKeyboard(cell)`, `arrow('ArrowRight')`, `pointerMove(x, y)`, `dropByKeyboard()`, `cancelGrab()`, or the whole pointer gesture with `dragTo(handle, x, y)`. Read results from `cells()` (`aria-colindex` / `aria-rowindex`) or from a controller captured with `onfirstcreate`. `realCells()` excludes the drop preview a target renders mid-grab. ARIA row and column indices are one-based; controller coordinates and `cellAt(x, y)` are zero-based. Assert ARIA positions after dropping: a held widget temporarily uses `role="group"` without grid indices. Use `[data-flexi-widget]` to find widgets in every state. For a drop flight, `mockFrames()` queues `requestAnimationFrame` so you can step it. Guide: https://flexiboards.dev/docs/guides/testing.md. The adapters' own suites in `packages/{react,svelte}/tests` are worked examples.
