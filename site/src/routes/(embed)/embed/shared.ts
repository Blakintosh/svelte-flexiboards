 ...
 * client-side, like React embeds always do: the board appears in its final state
 * rather than server-rendering a stand-in that then updates. Opted-in examples
 * either declare their whole layout (so the server renders the truth) or
 * deliberately show the suspense fallback. The examples viewer surfaces this as
 * a "Server-side rendering" feature chip.
 */
