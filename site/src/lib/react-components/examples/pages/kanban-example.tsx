...
import KanbanAddCard from '../kanban/kanban-add-card';
import KanbanCard, { type KanbanCardData } from '../kanban/kanban-card';
import KanbanColumnHead from '../kanban/kanban-column-head';
...
export default function KanbanExample() {
	...
	const onHeadPosition = useCallback((key: string, x: number | undefined, grabbed: boolean) => {
		setHeadPositions((current) => {
			...
			if (previous && previous.x === x && previous.grabbed === grabbed) return current;
			return { ...current, [key]: { x, grabbed } };
		});
	}, []);
	...
	const slots = useMemo(
		() =>
			computeSlots(
				SEED_ORDER,
				SEED_ORDER.map((key) => headPositions[key]?.x),
				SEED_ORDER.map((key) => headPositions[key]?.grabbed ?? false)
			),
		[headPositions]
	);
	...
	const syncCards = useCallback(
		(layout: FlexiLayout) => {
			...
			persist(lastLayout.current);
		},
		[persist]
	);

	function addCard(key: ColumnKey, title: string) {
		const target = cardTargets.current[key];
		if (!target) return;
	...
	function reset() {
		...
		pendingCards.current = seed;
		lastLayout.current = cloneLayout(seed);
		lastTotal.current = totalOf(seed);
		setCounts(countsOf(seed));
		setUndoLayout(null);
		setColumnOrder([...SEED_ORDER]);
		setHeadPositions({});
		cardTargets.current = {};
	...
	const cardsConfig = useMemo<FlexiBoardConfiguration>(
		() => ({
			...
			loadLayout: () => pendingCards.current,
			onLayoutChange: syncCards
		}),
		[syncCards]
	);

	const railConfig = useMemo<FlexiBoardConfiguration>(
		() => ({
			widgetDefaults: {
				draggability: 'full',
				resizability: 'none',
			...
			onLayoutChange: () => persist(lastLayout.current)
		}),
		[persist]
	);
	...
	return (
		<main className="bg-paper text-ink flex h-full min-h-0 w-full flex-col p-3 lg:p-6">
			...
			>
				<header className="flex shrink-0 items-baseline justify-between gap-3 px-4 pt-5 lg:px-8">
					<div className="flex min-w-0 flex-wrap items-baseline gap-x-4 gap-y-1">
						<h1 className="text-ink font-serif text-xl sm:text-2xl lg:text-[30px]">Sprint 14</h1>
						<p className="text-body text-[12px]">
							Cards move between four targets;
							<span className="hidden sm:inline">
								{' '}
								the headings are a second, independent board.
							</span>
						</p>
					</div>
					...
					<Button
						variant="outline"
						size="icon"
						className="bg-paper border-rule-soft size-[34px] shrink-0 rounded-full"
						onClick={reset}
						title="Reset board"
					>
					...
					<div className="flex min-h-0 flex-1 flex-col gap-3 overflow-auto px-4 pt-5 pb-3 lg:px-8">
						...
						*/}
						<FlexiBoard className="shrink-0" config={railConfig}>
							<FlexiTarget keyName="columns" className="gap-4" config={railTargetConfig}>
								{columnOrder.map((key) => (
									<FlexiWidget key={key} type="column" metadata={{ key, title: COLUMNS[key] }}>
										<KanbanColumnHead count={counts[key] ?? 0} onPosition={onHeadPosition} />
									</FlexiWidget>
								))}
							</FlexiTarget>
						</FlexiBoard>
						...
						*/}
						<div className="grid shrink-0 grid-cols-[repeat(4,minmax(216px,1fr))] items-start gap-4">
							{SEED_ORDER.map((key) => (
								...
								<div key={key} className="flex min-w-0 flex-col" style={{ order: slots[key] }}>
									<FlexiTarget
										keyName={key}
										onfirstcreate={(controller) => (cardTargets.current[key] = controller)}
						...
						{undoLayout ? (
							<>
								...
								<Button
									variant="ghost"
									size="sm"
									className="text-ink hover:bg-tint ml-auto h-7 shrink-0 gap-1.5 rounded-full px-2"
									onClick={undoArchive}
								>
									<Undo2 className="size-3.5" />
									<span className="ui text-xs">Undo</span>
								</Button>
							</>
						) : (
							<span className="min-w-0 truncate text-[11.5px] font-semibold">
								Archive<span className="hidden sm:inline">&nbsp;— drag a card here</span>
							</span>
						)}
					</FlexiDelete>
				</FlexiBoard>
