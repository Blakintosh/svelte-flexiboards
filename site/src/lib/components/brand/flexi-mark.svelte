<script lang="ts">
	/*
	  ...
	  vermillion — the widget that just moved. Bar height equals the gap x 2;
	  widths step 26/17/9 at large size, 18/12/6 in nav. The vermillion bar never
...
<div
	class="flex flex-col items-start {className}"
	style="gap:{g.gap}px"
	aria-hidden="true"
>
	{#each g.widths as width, i (i)}
		<div
			class={i === 2 ? 'bg-vermillion' : inverted ? 'bg-paper' : 'bg-ink'}
