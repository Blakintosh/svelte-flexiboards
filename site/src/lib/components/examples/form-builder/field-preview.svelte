...
<script lang="ts">
	...
	// Inert but real: disabled native controls at full opacity, so the canvas
	// reads as a form rather than a broken one.
	const control =
...
{:else if meta.kind === 'select'}
	<!-- Native <select>, chevron drawn beside it so the disabled control keeps
	     the live one's affordance. -->
