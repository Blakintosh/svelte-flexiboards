...
<div class="flex shrink-0 flex-col gap-2 lg:w-52">
	<span class="label text-faint text-[10px]">Fields</span>
	...
	<div class="flex flex-wrap gap-2 lg:flex-col lg:flex-nowrap">
		{#each FIELD_KINDS as spec (spec.kind)}
			...
			<FlexiAdd
				...
				class={'ui text-xs border-rule bg-tint-2 text-body hover:border-ink hover:text-ink focus-visible:outline-blue flex flex-1 basis-[calc(50%-0.25rem)] items-center gap-2 border px-2 py-2 text-left transition-colors duration-[120ms] focus-visible:outline-2 focus-visible:outline-offset-2 sm:basis-[calc(33.333%-0.4rem)] lg:basis-auto'}
			>
	...
	<FlexiDelete
		class={(deleter: FlexiDeleteController) =>
			cn(
				'ui text-xs border-rule bg-tint-2 text-faint flex items-center justify-center gap-2 border border-dashed px-3 py-3 text-center duration-[120ms] lg:mt-auto lg:flex-col lg:py-6',
