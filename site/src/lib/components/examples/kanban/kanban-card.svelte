<script module lang="ts">
	 ...
	 * renders comes from there, so a card survives export, localStorage and
...
<!--
	The card in hand is the only thing on the sheet that moves: a 1° tilt on
	the contents, leaving the library's own transform on the widget untouched.
-->
<div
...
>
	...
	<!-- Meta: a tag pill on the left, who and when in mono on the right. -->
	<div class="flex min-w-0 items-center gap-2 text-[10px]">
		<span class="bg-tint text-body shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold"
			>{card.tag ?? 'Task'}</span
		>
		<!-- The one data use of fx-accent: a date already past. -->
		<span
