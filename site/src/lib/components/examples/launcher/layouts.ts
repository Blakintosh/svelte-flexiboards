 ...
 * desktop / tablet / mobile buttons then swap the board's layout on their own).
 ...
 * there's no API to force one. So the switcher remaps the thresholds themselves,
 * which is ordinary public config: the pinned key drops to a min-width every viewport
 * satisfies, and the others go out of reach. Values are kept distinct so the controller's
