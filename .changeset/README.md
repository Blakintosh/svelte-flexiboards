...
The four packages are linked, so they release together at one version.

`@flexiboards/core`, `@flexiboards/svelte`, `@flexiboards/react` and `@flexiboards/testing` sit at 1.0.0 in their manifests and are not yet on npm at that version, so the first publish needs no changeset.
